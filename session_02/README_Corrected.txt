...
c) Two function graphs are entered in the plot, which assign a runtime of the program to the number of elements to be sorted, respectively for the algorithms
   ...
   MMS : Why didn't you fit the data with the expected scaling laws (using, e.g., a nonlinear least squares fit; gnuplot can do that) and plot the functions in the diagram?
...
e) The supposedly better sort algorithm is the Quick Sort algorithm, because it does the sorting of a large number of elements faster. However, it should be noted
   ...
   MMS: Where is the routine for generating "time_complexity.dat"? Or did you do that by hand?
        For small arrays, IS is usually faster than QS.
        2/3 points
...
2.2 Bug Hunting Exercise
    ...
    Adding PRINT statements to the code shows that the matrix elements are not correctly scanned for calculation in the loops with the index variables. In the
    example for the calculation of the ne-to-sw diagonals, the elements are in the order 2, 0, 7; 3, 4, 8; 1, 5, 0, where the elements separated by commas are
    multiplied. But instead of the 0 you would expect to multiply a 9 as the last digit. This can be achieved by changing the "ind" variable as noted in the
    program. For a correct calculation of the determinant, the second "ind" variable has to be changed as well, because there is also an error here, according to
    which the matrix elements are not passed through completely. Furthermore the ARGUMENT of ALLOCATE(ind(ARGUMENT)) was changed from 2*n-1 to 2*n, because now
    more memory should be allocated for the 6 instead of 5 elements in this array.

    MMS: 2/2 points

MMS: 2.1.2: You don't have to print to stdout. Instead of not allowing for output of more than 100 numbers, I would have rather overwritten the data contained in the input file with the sorted numbers, or would have created a new file, no matter how large N is. BTW: If you don't want to print your array components into a single line, you can print one component per line by using a do loop:
DO i = 1, N
  print*, input_data_dp(i)
END DO
